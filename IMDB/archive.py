    ...
    def update_archive(self, ind, evaluator):
        ...
        if ind not in self.archive and ind.distance_to_target <= 1:
            ...
            if len(self.archive) == 0:
                log.info(f"ind {ind.id} with seed {ind.seed} and ({ind.features['poscount']}, {ind.features['negcount']}, {ind.features['verbcount']}), performance {ind.ff}, sparseness {ind.sparseness} and distance {ind.distance_to_target} added to archive")
                ...
                # archive is full
                    ...
                    if c.distance_to_target > ind.distance_to_target:
                        log.info(f"ind {ind.id} with seed {ind.seed} and ({ind.features['poscount']}, {ind.features['negcount']}, {ind.features['verbcount']}), performance {ind.ff}, sparseness {ind.sparseness} and distance {ind.distance_to_target} added to archive")
                        log.info(f"ind {c.id} with seed {c.seed} and ({c.features['poscount']}, {c.features['negcount']}, {c.features['verbcount']}), performance {c.ff}, sparseness {c.sparseness} and distance {c.distance_to_target} removed from archive")
