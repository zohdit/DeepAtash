...
from .tig_utils import close_quietly, exec_quietly
...
class Vehicle:
    ...
    def disconnect(self):
        ...
        """
        for name, sensor in self.sensors.items():
            exec_quietly( lambda :sensor.disconnect(self.bng, self) )
    ...
    def detach_sensor(self, name):
        ...
        log.debug('before sensor.detach(self, name)')
        sensor.detach(self, name)
        log.debug('after sensor.detach(self, name)')
